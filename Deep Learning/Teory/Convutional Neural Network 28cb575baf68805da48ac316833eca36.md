...
![image.png](Convutional%20Neural%20Network/image.png)
...
![image.png](Convutional%20Neural%20Network/image%201.png)
...
![image.png](Convutional%20Neural%20Network/4c1b4491-d23b-4f65-9880-96691402da60.png)
...
![image.png](Convutional%20Neural%20Network/image%202.png)
...
![image.png](Convutional%20Neural%20Network/image%203.png)

**`Note`** that this diagram **simplifies the process for a single channel**. In practice, a filter’s **depth must match the input volume’s depth** (e.g., a $3×3×3$ filter for a $32×32×3$ RGB image). The convolution involves **element-wise multiplications across all channels**, and the results are **summed together**, along with a **single bias term**, to produce **one pixel** in the output feature map.
...
- **Kernel Size** ($K × K$): Specifies the **spatial dimensions** of each filter. Kernels are usually square and always extend across the **entire depth** $C_{in}$ of the input volume.
...
Given an input volume of size $H × W × C_{in}$, the number of learnable parameters for a convolutional layer with $C_{out}$ filters and a kernel size of $K × K$ is:

$\text{Number of learnable parameters} = (K × K × C_{in}) × C_{out} + C_{out}$

**Explanation**

- There are $C_{out}$ distinct filters (kernels) in the layer.
- Each filter has spatial dimensions $K × K$ and extends through the full depth of the input volume ($C_{in}$). So, each filter has K × K × Cin weights.
...
![image.png](Convutional%20Neural%20Network/image%204.png)
...
![image.png](Convutional%20Neural%20Network/image%205.png)
...
![image.png](Convutional%20Neural%20Network/image%206.png)

The main **hyperparameters** of pooling layers are:

- **Pool Size** ($K$): The spatial dimensions of the pooling window (e.g., 2 × 2).
...
The most common configuration is a **pool size** of $K = 2$ (a $2×2$ window) with a **stride** of $S = 2$, meaning that 75% of activations are discarded at each pooling step
...
Pooling reduces the spatial dimensions of the feature maps, which can be problematic for tasks requiring **precise localization**, such as **semantic segmentation**, where each pixel must be assigned a specific class label.
    ...
    ![image.png](Convutional%20Neural%20Network/image%207.png)
...
![image.png](Convutional%20Neural%20Network/image%208.png)
