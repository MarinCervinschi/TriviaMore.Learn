...
A **time series** is a **set of observations** $\{xₜ\}_{t \in \tau}$  collected over time, that spans from a starting time $t$to an ending time $\tau$, where $\tau$ is the total number of observations. 
...
A time series is **stationary** if its **statistical properties** — such as mean and variance — **remain do not change over time**. Conversely, if the **trend is non-constant**, the time series is considered **non-stationary.**
...
![image.png](Recurrent%20Neural%20Networks/image.png)
...
![image.png](Recurrent%20Neural%20Networks/image%201.png)
...
![image.png](Recurrent%20Neural%20Networks/image%202.png)
...
![image.png](Recurrent%20Neural%20Networks/image%203.png)
...
![image.png](Recurrent%20Neural%20Networks/image%204.png)
...
![image.png](Recurrent%20Neural%20Networks/image%205.png)
...
![image.png](Recurrent%20Neural%20Networks/image%206.png)
...
This repeated multiplication of the *same matrix* $W$ can two numerical instabilities:
...
![image.png](Recurrent%20Neural%20Networks/image%207.png)

Here $\odot$ denotes **element-wise multiplication**

![image.png](Recurrent%20Neural%20Networks/0a1cf27c-9a2b-4a75-b5cf-7cbe1bbae7b9.png)
...
- **Candidate State**
    ...
    $\begin{cases} ... \\[6pt]\tilde{C}= \tanh\!\left(x^{(t)} U_C + h^{(t-1)} W_C\right) \\... \end{cases}$
...
- **Memory Cell’s update and hidden state**
    ...
    $\begin{cases}... \\[6pt]C^{(t)} = C^{(t-1)} \odot f + \tilde{C} \odot i \\[6pt]h^{(t)} = \tanh\!\left(C^{(t)}\right) \odot o \end{cases}$
    
    First equation computes the u**pdate for memory cell** $C$. Here:
    ...
    So I will pass to the next cell two things, HT and CT. Because I'm both needed. HT is used for computing I, F, O, and C tilde, and CT is needed for computing CT plus one
