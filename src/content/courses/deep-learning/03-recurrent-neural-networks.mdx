...
- **Memory Cell’s update and hidden state**
    ...
    So I will pass to the next cell two things, $h^{(t)}$ and $C^{(t)}$. Because I'm both needed. $h^{(t)}$ is used for computing I, F, O, and C tilde, and $C^{(t)}$ is needed for computing $C^{(t+1)}$.
