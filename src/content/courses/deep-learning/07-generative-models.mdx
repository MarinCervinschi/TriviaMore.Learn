...
![image.png](/images/deep-learning/07-generative-models/image.png)
...
$$
\log p(x) - \underbrace{D_{KL} [q(z|x) \parallel p(z|x)]}_{\substack{\text{KL divergence between the proxy} \\ \text{and the real posterior distributions}}} = \underbrace{\mathbb{E}_{z \sim q(z|x)} [\log p(x|z)] - D_{KL} [q(z|x) \parallel p(z)]}_{\text{ELBO: Evidence Lower BOund}}
$$
...
![image.png](/images/deep-learning/07-generative-models/image%201.png)
...
![image.png](/images/deep-learning/07-generative-models/image%202.png)

**Components:**

- **Encoder network** $q(z | x)$: Given an input $x$, the encoder estimates the posterior distribution $p(z|x)$ over latent variables.  In practice, this is usually modeled as a multivariate Gaussian: $x \to f_{\text{ENC}}(x; \theta) = [\mu(x), \sigma^2(x)]$ such that $z \sim \mathcal{N}(z \mid \mu(x), \sigma^2(x))$. The network outputs two vectors, $\mu(x)$ and $\sigma^2(x)$, representing the parameters of the approximate posterior.
...
This term measures how much the approximate posterior $q(z|x)$ produced by the encoder diverges from the **prior distribution** $p(z)$. 
...
![image.png](/images/deep-learning/07-generative-models/image%203.png)
...
![image.png](/images/deep-learning/07-generative-models/image%204.png)
...
![image.png](/images/deep-learning/07-generative-models/image%205.png)
...
![image.png](/images/deep-learning/07-generative-models/image%206.png)
...
![image.png](/images/deep-learning/07-generative-models/image%207.png)
...
- **Pros:**
    ...
    - No MCMC needed
- **Cons:**
    - Trickier and more unstable to train
...
![image.png](/images/deep-learning/07-generative-models/fb9f3b31-08be-4fa9-88f5-589126177369.png)
...
![image.png](/images/deep-learning/07-generative-models/image%208.png)
...
![image.png](/images/deep-learning/07-generative-models/image%209.png)
...
![image.png](/images/deep-learning/07-generative-models/image%2010.png)
...
![image.png](/images/deep-learning/07-generative-models/image%2011.png)
...
| Concept | Description |
|---|---|
| **Generative modeling** | Learn $p(x)$ to sample new data and assess likelihood; enables anomaly detection |
| **Latent variable models** | Model $p(x) = \int p(x \mid z) \, p(z) \, dz$; latent variables $z$ capture high-level factors of variation |
| **Variational inference** | Learn a proposal $q(z \mid x) \approx p(z \mid x)$ by minimizing KL divergence; avoids blind Monte Carlo sampling |
| **ELBO** | $\mathbb{E}[\log p(x \mid z)] - D_{KL}[q(z \mid x) \| p(z)]$; tractable lower bound on $\log p(x)$ |
| **VAE** | Encoder outputs $\mu, \sigma^2$ of a Gaussian posterior; decoder reconstructs from sampled $z$; trained via ELBO |
| **Reparameterization trick** | $z = \mu + \epsilon \cdot \sigma$, $\epsilon \sim \mathcal{N}(0, I)$; makes sampling differentiable for backpropagation |
| **GAN** | Generator $G(z)$ vs. discriminator $D(x)$; minimax game — no explicit density needed |
| **GAN vanishing gradients** | When $D$ is too strong, $G$ gets near-zero gradients; fix: maximize $\log D(G(z))$ instead of minimizing $\log(1 - D(G(z)))$ |
| **DCGAN** | CNNs in both $G$ and $D$; smooth latent interpolation and semantic arithmetic in latent space |
| **CycleGAN** | Unpaired image-to-image translation via cycle consistency: $x \to G_{A \to B}(x) \to G_{B \to A}(\cdot) \approx x$ |
| **Progressive GAN** | Trains from low-res to high-res by progressively adding layers; enables 1024x1024 generation |
| **StyleGAN** | Maps $z \to w$ via MLP for disentangled control; AdaIN injects style; noise adds fine details |