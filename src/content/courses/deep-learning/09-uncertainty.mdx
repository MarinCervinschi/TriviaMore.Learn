...
1. **Overconfident Predictions**
    ...
    ![image.png](/images/deep-learning/09-uncertainty/bb404505-2c0c-4638-a7f2-efe4a513cf60.png)
...
2. **OOD Detection**
    ...
    ![image.png](/images/deep-learning/09-uncertainty/2e3bed8d-05d5-467e-a0f3-0c801779aeaa.png)
...
In autonomous driving, placing an adversarial sticker on a "Stop" sign could trick the vehicle's perception system into misinterpreting it as a "Speed Limit" sign, potentially causing an accident.
    ...
    ![image.png](/images/deep-learning/09-uncertainty/d93fa3d8-34d0-40c5-b646-2067a91c4f99.png)
...
![image.png](/images/deep-learning/09-uncertainty/image.png)
...
![image.png](/images/deep-learning/09-uncertainty/image%201.png)
...
![image.png](/images/deep-learning/09-uncertainty/image%202.png)
...
![image.png](/images/deep-learning/09-uncertainty/3adf311f-1d2f-4bd8-90c0-f83686d6bf42.png)
...
$$
\mathcal{L}_B = \sum_{(x,y)\in B} - \log p(y | x, w) + \pi \, KL(q_\theta(w) \parallel p(w))
$$
...
![image.png](/images/deep-learning/09-uncertainty/image%203.png)
...
![image.png](/images/deep-learning/09-uncertainty/image%204.png)
...
![image.png](/images/deep-learning/09-uncertainty/image%205.png)
...
Experiments show that deeper networks, such as **ResNet**, achieve higher accuracy than older architectures like **LeNet**. However, their confidence scores, calculated as the average maximum probability:

$\frac{1}{N} \sum_{i=1}^N \max_{k \in \{1,...,K\}} p_\theta(y=k|x_i)$

are often heavily concentrated near 1.0.

![image.png](/images/deep-learning/09-uncertainty/image%206.png)
...
In miscalibrated models, this relationship breaks down.
</Callout>
...
![**LeNet (left) vs ResNet (right)**](/images/deep-learning/09-uncertainty/image%207.png)
...
![image.png](/images/deep-learning/09-uncertainty/image%208.png)
...
![image.png](/images/deep-learning/09-uncertainty/image%209.png)
...
| Concept | Description |
|---|---|
| **Aleatoric uncertainty** | Irreducible noise in the data (sensor noise, stochastic processes); cannot be reduced with more data |
| **Epistemic uncertainty** | Model ignorance due to limited/biased training data; reducible by collecting more data or improving the model |
| **Deep Ensembles** | Train $M$ independent networks with different initializations; disagreement among predictions reflects epistemic uncertainty |
| **Bayesian Neural Network** | Weights are distributions $q_\theta(w) \approx p(w \mid D)$ instead of point estimates; explicitly models parameter uncertainty |
| **Bayes by Backprop** | Variational inference for BNNs: learn $\mu, \sigma$ per weight; minimize KL divergence + NLL via reparameterization trick |
| **MC Dropout** | Keep dropout active at test time; $T$ stochastic passes approximate Bayesian inference without retraining |
| **Overconfidence** | Modern DNNs assign high softmax probabilities even to wrong predictions; softmax scores are not calibrated probabilities |
| **Calibration** | A well-calibrated model’s predicted confidence matches its empirical accuracy (e.g., 80% confidence $\to$ 80% correct) |
| **Reliability diagram** | Plots predicted confidence vs. actual accuracy per bin; perfect calibration follows the $45^\circ$ diagonal |
| **ECE** | $\sum_m \frac{\lvert B_m \rvert}{n} \lvert acc(B_m) - conf(B_m) \rvert$; single number summarizing miscalibration across bins |
| **Temperature scaling** | Post-training fix: divide logits by $T > 1$ before softmax; smooths confidence without changing accuracy |
